...
import "./pedidosasepararstyles.css";
...
function PedidosAseparar() {
  const { userName, filialUsuario } = useContext(AuthContext);
  const navigate = useNavigate();
  const { state } = useContext(PedidoContext);
  ...
  useEffect(() => {
    const handleSearch = async () => {
      ...
      api
        .get(
          `Separacao/ObtemPedidosASeparar?usuario=${usuario}&filial=${filial}`
        )
        .then((response) => {
  ...
  return (
    <div className="containerSEPARAÇÃOEMANDAMENTO">
      <div className="gridSEPARAÇÃOEMANDAMENTO">
        <div className="texto2">PedidosAseparar</div>
...
export default PedidosAseparar;
